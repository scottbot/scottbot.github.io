...
categories: ["method"]
tags: ["complexity", "data analysis", "digital humanities", "gephi", "methodologies", "network analysis", "networks demystified", "social networks"]

original_url: ""
archive_url: ""
archive_date: "2015-04-25"
archive_timestamp: "20150425231859"

language: "en"
comments_preserved: false
comment_count: 0

source_pdf: "Weingart - 2015 - Networks Demystified 9 Bimodal Networks.pdf"
source_pdf_sha256: "429236c6aed2a0978a52aab64ea26ffed4072aefdc430610f35fdfa77564fe31"
extraction_date: "2026-08-02"
extraction_tool: "claude scholarly-blog-html-to-markdown skill (from Wayback print PDF)"
extraction_notes: |
  Post date confirmed as 2015-01-21 from the Wayback capture of the blog's monthly archive
  page (scottbot.net), which prints the dateline the single-post print omitted.
  DISAMBIGUATION: this is instalment NINE of the Networks Demystified series
  ("Bimodal Networks", category "in method"), distinct from instalments 3, 4,
  5, 7 and 8, which were converted separately
  (2012-networks-demystified-3-the-power-law-rant,
  2013-networks-demystified-4-co-citation-analysis,
  2013-networks-demystified-5-communities-pagerank-and-sampling-caveats,
  2013-09-23-networks-demystified-7-doing-co-citation-analyses,
  2013-11-05-networks-demystified-8-when-networks-are-inappropriate).
  Source is a Firefox/cairo browser print-to-PDF (13 pages, printed 4/4/2026)
  of Wayback snapshot 20150425231859 of the scottbot.net/HIAL post
  (Independent Publisher theme).
  PROVENANCE CAVEATS: (1) the theme printed no dateline, so no exact post date
  is recoverable from this artifact; the post quotes a tweet dated
  "January 20, 2015" as "recent" and the snapshot is 2015-04-25, so the post
  falls between those dates. post_date is therefore the bare year 2015 (which
  also matches the source PDF's filename) and the output folder uses a
  year-only slug. (2) The print header truncates the snapshot URL
  ("https://web.archive.org/web/20150425231859/http://www.scottbot.net...") and
  the print contains no self-referential permalink; a search of the link
  annotations and text of every other PDF in this corpus found no
  cross-reference to this post, and web.archive.org is unreachable from this
  sandbox. archive_url and original_url are therefore left EMPTY rather than
  guessed; the timestamp itself (20150425231859 = 2015-04-25) is certain.
  Chrome stripped: print header/footer (running title, truncated URL, "N of
  13", "4/4/2026, 1:32 PM"), the Wayback banner line, the "in method" category
  line (-> YAML categories), the post title (re-emitted as the H1, restoring
  the colon the print breaks across two lines), the Genericons speech-bubble
  glyph and the "Write a Comment" heading, the RELATED CONTENT BY TAG cloud
  (8 tags -> YAML), and the "Independent Publisher empowered by WordPress"
  footer credit. No comments were printed (comment_count 0).
  COLOUR IS NOT REPRESENTABLE IN MARKDOWN: in the source the names of the
  three people (Alice, Bob, Carol) are set in green and the names of the three
  clubs (Network Club, We Love History Society, No Adults Allowed Club) in
  blue, matching the node colours of the figures. Markdown has no colour, and
  no HTML was injected, so those runs are emitted as plain text; their wording
  is unchanged. This is the only presentational feature of the post that the
  conversion does not carry over.
  The embedded tweet (Shawn Graham's request that prompted the post) is
  preserved as a blockquote: the tweet text and the "— Shawn Graham
  (@electricarchaeo) January 20, 2015" attribution, both printed in italic by
  the theme and therefore emitted as one italic blockquote, with the
  @scott_bot and permalink anchors intact.
  Images: 7 files extracted from the embedded rasters at native resolution.
  Five of the seven straddle a print page break; in each case the uncropped
  embedding was used rather than the page-break crop. Five of them (img-001,
  img-002, img-003, img-004 and img-007) carry an /SMask alpha channel and
  were composited over white, as the browser rendered them; img-004 is a
  greyscale raster and was converted to RGB before compositing so its
  transparent background does not come out black. img-005 and img-006 are
  opaque and were saved directly. No image
  carries a link annotation, so none is hyperlinked to its full-size upload.
  Captions are kept as italic caption paragraphs exactly where the print puts
  them, including their "[via]" hyperlinks.
  ...
  verbatim: "or the our Historian's Macroscope chapter", the mismatched
  quotation marks in '"centrality".' (the post opens and closes the quotation
  with a left double quote), the caption "used to create a unipartite (left)
  and a bipartite (right) network" (the figure actually shows the bipartite
  network on the left), "you'll be lead astray", "a  high degree" and "Digital
  Humanists.  I've found" (double spaces), and "for a finding bimodal
...
[Surely](https://www.youtube.com/watch?v=0A5t5_O8hdA), you’re thinking, bimodal networks, with only two categories, should be fine! Wellllll, yes and no. You don’t bump into the same aggregation problem you do with very multimodal networks; instead, you bump into technical and mathematical issues. These issues are why I [often](http://www.scottbot.net/HIAL/?p=6526) warn nontechnical researchers [away](http://www.scottbot.net/HIAL/?p=39600) from bimodal networks in research. They’re not theoretically unsound, they’re just difficult to work with properly unless you know what changes when you’re working with these complex networks.

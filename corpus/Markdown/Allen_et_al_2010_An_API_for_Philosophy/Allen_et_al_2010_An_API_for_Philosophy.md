...
container_title: "APA Newsletter on Philosophy and Computers"
...
year: 2010
language: "en"
source_pdf: "Allen_et_al__-_2010_-_An_API_for_Philosophy.pdf"
source_pdf_sha256: "a17e6994ca6b1715c8fb0adad7ecf4a44e99726f8d149ba2135257a664ea6a1c"
page_count: 2
extraction_date: "2026-04-26"
extraction_tool: "claude scholarly-pdf-to-markdown skill"
extraction_notes: |
  Source is an Adobe Paper Capture (OCR-scanned) PDF; each page is one
  embedded JPEG with an OCR text layer that contains predictable scan
  artifacts (e.g. "AP/" for "API", "APls"/"APis" for "APIs", "Figure I"
  for "Figure 1", "Fal/" for "Fall", "JO" for "10"). The body text below
  reproduces the article as printed in the source, with these OCR
  artifacts corrected back to the printed glyphs.

  Per the user's instruction, transcription begins at "An API for
  Philosophy" on page 1; the truncated end-of-previous-article footnote
  list (items 1-8) that occupies the top of column 1 on page 1 is not
  part of this article and is not reproduced.

  Figure 1 (a network graph titled "Part of the related ideas network in
  the neighborhood of 'Turing test'") is preserved as a 300 DPI PNG
  cropped from the page raster (images/fig-001.png), since the source
  PDF stores the page as a flat image and there is no separate vector
  figure to extract.

  Italics in the running text (the words "what" and "how" in the first
  paragraph; the journal title "Synthese" and the proceedings title
  "Proceedings of 21st FLAIRS" in the references) are preserved.
bibkey: "allenAPIPhilosophy2010"
---

# An API for Philosophy

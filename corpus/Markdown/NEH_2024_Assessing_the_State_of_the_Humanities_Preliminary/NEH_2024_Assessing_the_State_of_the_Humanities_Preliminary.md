---
title: "Assessing the State of the Humanities"
subtitle: "Preliminary Feedback on a Proposed Data-Driven Research Grant Program"
authors:
...
contributors:
  - { family: "Alpert-Abrams", given: "Hannah", role: "prepared by", title: "Senior Program Officer" }
  - { family: "Unger", given: "Jessica", role: "prepared by", title: "Program Analyst (Office of Challenge Programs)" }
  - { family: "Weingart", given: "Scott B.", role: "support", title: "Director" }
  - { family: "Lazopoulos", given: "George", role: "support", title: "Senior Program Officer (Division of Public Programs)" }
  - { family: "Rodriguez", given: "Lutie", role: "support", title: "Data Coordination and Enablement Officer" }
  - { family: "Skinner", given: "Benjamin", role: "support", title: "Humanities Data Scientist" }
...
source_pdf: "SOH_Prelim.pdf"
source_pdf_sha256: "47562331f1cd9f5bf7be2121c9357d4d0dde77085078893829f3b001607245cf"
page_count: 13
extraction_date: "2026-08-02"
extraction_tool: "claude scholarly-pdf-to-markdown skill"
extraction_notes: |
  Digital-native LaTeX PDF (MiKTeX/dvipdfmx, "LaTeX with hyperref"), single column,
  13 pages. Metadata was taken from the document itself; no network/Crossref lookup
  was available, and the PDF carries no DOI, ISBN, or embedded title metadata.
  publication_type is "report".

  No personal byline is printed on the title page: the work is issued by the Office of
  Data and Evaluation, National Endowment for the Humanities, and that office is
  recorded as the corporate author. The named staff on the credits page ("Prepared by"
  / "With support from", including Scott B. Weingart as Director) are recorded under
  `contributors` with their printed roles, and the credits page is also transcribed
  verbatim in the body.

  Structure: PDF page 1 is a designed front cover (title, subtitle, date and office set
  in white type over a full-bleed blue image) and PDF page 13 is a designed back cover;
  both are rasterized at 200 DPI into images/ (fig-001, fig-002). The document prints its
  title block twice - once on the cover and again on the LaTeX title page (PDF page 2),
  which omits the cover's line "A Report from the" - and both are transcribed verbatim in
  that order rather than merged; the cover instance carries the document's single `#`
  heading and the title-page repeat is set in bold. PDF pages 1-3 carry no
  printed folio; their page anchors are `<!-- page cover -->`, `<!-- page title -->`
  and `<!-- page credits -->`. Body pages carry printed folios 1-5, the appendices A1-A2
  and B1-B2, and those printed folios are used for the remaining anchors.

  Headings are set in Helvetica against a Georgia body face; the three printed heading
  sizes (17.2 / 14.3 / 12.0 pt) map to `##` / `###` / `####`. The two-line heading
  "APPENDIX B: RELATED GRANT PROGRAMS AND INITIATIVES" was collapsed to one line.
  The running footer ("Assessing the State of the Humanities" + folio) was removed;
  folios survive as page anchors.

  Both appendix tables have a real text layer and are reconstructed as GitHub-flavored
  Markdown tables; no table was rasterized or dropped. Each appendix runs over two
  printed pages and the source repeats the column header on the continuation page, so
  each printed page is rendered as its own GFM table with the header as printed, which
  keeps the page anchors outside the table markup. Cells that wrap over several printed
  lines are joined into one cell.

  Hyperlinks: the grant-number cells of Appendix A and the program-name cells of
  Appendix B carry external link annotations, reconstructed as Markdown links with the
  exact target recorded in the PDF. Note a source defect preserved as printed: the link
  on grant AH-268665-19 points at the award page for AH-266283-19 in the original PDF;
  this is reproduced, not corrected. Three internal hyperref cross-references on printed
  page 2 (to "Appendix A, Related NEH Awards" and "Appendix B, Related Grant Programs
  and Initiatives") have no external target and are rendered as plain text.

  Line-break hyphens were removed and the words rejoined; genuine compounds that also
  occur mid-line (e.g. "Large-scale") keep their hyphen. Smart quotes, en dashes and
  em dashes are preserved as printed. No OCR was used, no figures other than the two
  cover images exist, and there are no footnotes, endnotes, equations, or bibliography
  in this document. MANIFEST.txt included.
bibkey: "Public_report"
---

![Front cover: the report title, subtitle, date, and issuing office set in white type on a blue field bearing the seal of the National Endowment for the Humanities.](images/fig-001.png)
...
![Back cover: the seal and wordmark of the National Endowment for the Humanities in white on a blue field.](images/fig-002.png)

...
tags: ["data analysis", "dhconf", "digital humanities", "disciplinarity", "diversity", "gender", "scholarly communication"]
...
archive_date: "2015-07-19"
archive_timestamp: "20150719233052"

language: "en"
comments_preserved: true
comment_count: 8

source_pdf: "Weingart - 2015 - Acceptances to Digital Humanities 2015 (part 4).pdf"
source_pdf_sha256: "fe681d45a5344c786fb2eac6989248e0f73b1a8ded64160951e25d2ff1ffca5c"
extraction_date: "2026-08-02"
extraction_tool: "claude scholarly-blog-html-to-markdown skill (from Wayback print PDF)"
extraction_notes: |
  Post date confirmed as 2015-06-28 from the Wayback capture of the blog's monthly archive
  page (scottbot.net), which prints the dateline the single-post print omitted.
  Source is a browser print (2026-04-04) of Wayback snapshot 20150719233052
  of http://www.scottbot.net/HIAL/?p=41375 (post id recovered from the
  snapshot's own reply permalinks, ?p=41375&replytocom=...; the print header
  truncates the URL).
  POST DATE IS YEAR-ONLY: the theme printed no dateline. The post says it
  "showed yesterday" what part 3 showed, quotes a tweet of June 27, 2015, and
  its first five comments are dated June 29, 2015, so it is from on or just
  before June 29, 2015. The exact day is undeterminable offline; folder uses
  the <year>-<slug> form and the caveat is flagged in review notes.
  Print header/footer, Wayback banner, "Write a Comment" headings and their
  Genericons glyphs, "Reply to ..." links, comment avatars, the RELATED
  CONTENT BY TAG cloud (moved to YAML), and the theme footer credit stripped
  as chrome. Category "personal research" (?cat=57).
  BROKEN EMBED: as in other prints of this blog, the snapshot leaked
  fragments of a raw flickr-attribution caption down the left margin of pages
  3-4 - thin 82x82 / 382x21 raster strips at x=36-43 plus the text fragments
  '"font-', '574596/"', 'ickr</a>' (ArialMT 11.2, partly off-page), with link
  annotations to
  farm1.staticflickr.com/474/19504574596_2e16dca0bf_b.jpg. The image itself
  did not render in the snapshot; the fragments sit outside the article
  column and are stripped as broken markup, with the URL recorded here for
  provenance. No placeholder is inserted in the body.
  Images: 3 chart files, none hyperlinked in this print. img-003 (the tall
  topic-by-gender chart) prints across pages 6-8; its complete 528x1820
  raster is embedded on pages 6 and 7 and was extracted once. Figure captions
  (Figure 3's runs to six lines) reassembled as italic caption paragraphs.
  UNDERLINING AND STRIKETHROUGH: the text layer of the print drops both, but
  the strokes survive as vector rules and were recovered, classified by their
  height within the line (baseline = underline, mid-line = strikethrough) and
  re-emitted as <u>...</u> and ~~...~~. Three passages are underlined (the
  "if a peer reviewer sees the name 'Mary'..." question on p3, "there's a very
  clear bias against submissions by people with names non-standard to the US"
  on p5, and "It turns out that the gender bias in acceptance ratios is
  entirely accounted for by the topical bias" on p9). One passage is struck
  through: ", because by definition these are the topics that men publishing
  at DH conferences find less interesting or relevant" on p9 - the author's
  own post-publication retraction, immediately followed by footnote 3, "see
  Jacqueline Wernimont's comment below", and matching his reply to that
  comment ("I'll do some editing and point to your comment here").
  The post's three footnotes (superscripts after "...the zillionth post.",
  "...more often rejected", "...less interesting or relevant") converted to
  Markdown footnotes [^1]-[^3]; return arrows stripped.
  Two tweets (Heather Froehlich, June 27, 2015) preserved as blockquotes in
  the theme's italic styling with their permalinks.
  Justified-print end-of-line hyphenation dehyphenated (joins logged in
  review notes). Hyperlinks reconstructed from PDF link annotations; none
  guessed. Source typos and stylings preserved ("What I do have the titles
  and author names", "a large percentage of of those responsible", "women do
  fair worse", "guessed to women made it through", the literal asterisks in
  "*drum roll*", and the commenters' lowercase "i").
  All 8 reader comments preserved with nesting to depth 3; the three numbered
  points in jwernimont's comment keep their line breaks.
...
This is what I’m banking on for peer review data and bias. For the majority of my data on DH conferences, [Nickoal Eichmann](http://nickoal.com/) and I have been going in and hand-coding every single author with a gender that we glean from their website, pictures, etc. It’s quite slow, far from perfect (see [my note](http://www.scottbot.net/HIAL/?p=41355#note-41355-1)), but it’s at least more sensitive than the brute force method, we hope to improve it quite soon with user-submitted genders, and it gets us a rough estimate of gender ratios in DH conferences.
...
*However*, it turns out that the entire gender bias effect we see is explained by the topical bias [I already covered the other day](http://www.scottbot.net/HIAL/?p=41347). (Scroll down for the rest of the post.)
...
No. Of course not. I already [showed yesterday](http://www.scottbot.net/HIAL/?p=41347) that 46% of attendees to DH2015 are women, whereas only 35% of authors are. What it means is the bias against topics is gendered, but in a peculiar way that actually may be (relatively) easy to solve, and if we do solve it, it’d also likely go a long way in solving that attendee/authorship ratio too.

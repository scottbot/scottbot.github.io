...
categories: ["theory"]
tags: ["big data", "complexity", "data analysis", "diffusion", "digital humanities", "disciplinarity", "history", "history of science", "interdisciplinarity", "literary history", "macroanalysis", "methodologies", "scientonomy", "social science", "theory", "two cultures"]

original_url: "http://www.scottbot.net/HIAL/"
archive_url: ""
archive_date: "2014-11-26"
archive_timestamp: "20141126082102"

language: "en"
comments_preserved: false
comment_count: 0

source_pdf: "Weingart - 2014 - Bridging Token and Type.pdf"
source_pdf_sha256: "fb7d4dd3e8c37ad5bef07fb29ed9e84b76bee9a389bdcdb7057cdd9681f29aa7"
extraction_date: "2026-08-02"
extraction_tool: "claude scholarly-blog-html-to-markdown skill (from Wayback print PDF)"
extraction_notes: |
  Post date confirmed as 2014-01-18 from the Wayback capture of the blog's monthly archive
  page (scottbot.net), which prints the dateline the single-post print omitted.
  Source is a browser print (2026-03-31) of Wayback snapshot 20141126082102 of
  the scottbot.net/HIAL post. PROVENANCE CAVEATS: (1) the theme printed no
  dateline; the post's images live in wp-content/uploads/2014/01, indicating
  January 2014, but the exact day is undeterminable offline — post_date is the
  bare year and the folder uses a year-only slug, flagged in review notes.
  (2) No comments were printed, so no reply-permalink reveals the post's
  ?p= id: the print header truncates the snapshot URL, original_url records
  the blog root and archive_url is left empty; archive_timestamp is exact.
  Print header/footer, Wayback banner, "Write a Comment" heading, the RELATED
  CONTENT BY TAG cloud (16 tags moved to YAML), and the theme footer credit
  stripped as chrome. Category "theory" from the printed "in theory" line
  (?cat=33).
  Images: 4 files at native raster resolution. img-001 is the theme's
  featured-image banner (page 1), pixel-identical to the in-body Token/Type
  illustration, so one file serves both occurrences (in-body occurrence keeps
  its hyperlink to the full-size tokentype.png upload). img-002 (Science 26th
  Edition cartoon, linked to its upload via the print's annotation), img-003
  (perceived-affordances photo, linked to its upload), img-004 (Manhattan
  locals/tourists photo map, linked to its upload). Captions preserved as
  italic caption paragraphs, [via] links kept (tinypic, danlockton.com,
  flickr "Eric Fischer").
  The post's six numbered footnotes (superscript references printed as
  separate tiny digits; a "Notes:" list at the end with return-arrows)
  converted to Markdown footnotes [^1]-[^6]: superscript positions restored
  from the print's marker coordinates, return-arrows (↩) stripped. The
  stray space printed before the comma in "Stanley Milgram , where" absorbed
  the superscript 4; rendered as "Milgram[^4],". The two Wikipedia
  block-quoted definitions (Nomothetic/Idiographic), the Saussy quotation,
  and the two Underwood quotations rendered as blockquotes with single
  italic spans, per the print's italics. Justified-print end-of-line
  hyphenation dehyphenated (joins logged in review notes). Hyperlinks
  reconstructed from PDF link annotations; none guessed. Source oddities
  preserved: "you'd be be counting", "“Two Culture” divide",
  "“Pyschological Maps of Paris”", the missing place before the period in
  Milgram 1982 ("289–309." with publisher absent).
...
[^5]: If you’re interested in more thoughts on this subject specifically, I wrote a bit about it in relation to single-authorship in the humanities [here](http://www.scottbot.net/HIAL/?p=25594)

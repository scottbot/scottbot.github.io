...
authors:
  - { display: "Scott Weingart", url: "http://www.scottbot.net/" }
  - { display: "Jessica Otis", url: "http://orcid.org/0000-0001-5519-8331" }
post_date: "2016-01-05"
post_date_visible: "January 05, 2016"

blog_title: "Six Degrees of Francis Bacon"
blog_url: "https://6dfb.tumblr.com/"
blog_platform: "Tumblr"

categories: []
tags: ["networkingwomen", "gender"]
...
source_pdf: "Weingart and Otis - 2016 - Gender Inclusivity in Six Degrees.pdf"
source_pdf_sha256: "f3aea1b99151bdd086e7b732a49c6fc57f32d5a2f0e1cce19cefc4f6dbd98939"
extraction_date: "2026-08-02"
extraction_tool: "claude scholarly-blog-html-to-markdown skill (from browser print PDF of the live post)"
extraction_notes: |
  DIFFERENT SOURCE FROM THE REST OF THIS BATCH. This post is not from the
  scottbot irregular and not from the Wayback Machine: it is a Firefox/cairo
  browser print-to-PDF (4 pages, printed 4/5/2026) of the LIVE Tumblr post at
  https://6dfb.tumblr.com/post/136678327006/gender-inclusivity-in-six-degrees,
  the project blog of Six Degrees of Francis Bacon (Tumblr, "Minimal Theme" by
  Artur Kim). There is therefore no Wayback banner and no archive URL:
  archive_url, archive_date and archive_timestamp are left empty, and the
  print header carries the live URL (truncated), while the permalink itself is
  recovered in full from the link annotations on the printed title and the
  "3 notes" link.
  CO-AUTHORED: the print shows a joint byline under the title — "Scott
  Weingart and Jessica Otis" — with Weingart's name linked to scottbot.net and
  Otis's to her ORCID record. Both authors are recorded in YAML.
  post_date comes from the printed dateline "January 05, 2016" above the title
  (post_date_visible preserves the printed form).
  Chrome stripped: print header/footer; the Tumblr "Follow" / "Join Tumblr"
  buttons; the site navigation list (Home, Project Overview, Team, Grants and
  Support, In the News, Explore the Network, Submit a post, Archive, Mobile,
  RSS); the search box; the site banner image (linked to the blog root); the
  "3 notes" link; the printed title (re-emitted as the H1); the byline
  (-> YAML authors); the "Tags: networkingwomen gender" line (-> YAML tags);
  the notes list; and the footer (Creative Commons statement, "Powered by
  Tumblr", theme credit).
  NOTES / COMMENTS: Tumblr calls reblogs and likes "notes". The print's note
  list has four entries — "alliterative-endlessknot reblogged this from 6dfb",
  "alliterative-endlessknot liked this", "blherrou liked this", and "6dfb
  posted this" (the counter above reads "3 notes", excluding the post's own
  entry). None of them carries any reader text, so they are the Tumblr
  equivalent of pingbacks/likes rather than comments: stripped per the
  playbook's pingback rule, and comments_preserved is false with
  comment_count 0. Their existence is recorded here so nothing is silently
  lost.
  The extraction_tool string departs from this batch's default
  ("... (from Wayback print PDF)") because this print is of the live site, not
  of a Wayback snapshot.
  Images: 5 files at native raster resolution (504 px wide as served), all of
  them the post's own charts and network visualizations; none carries a link
  annotation, so all are plain images. The last one, labelled "Comparing
  network of men and women", prints as the women-only layout again in a
  fainter rendering — most likely an animated comparison of which the print
  captured a single frame; only what the print contains is reproduced, and the
  alt text says so rather than describing a comparison the file does not show. The site banner (903x302) and the four
  16-20 px note avatars were skipped as chrome. The post gives no figure
  captions; the running text labels three of the visualizations ("Everyone:",
  "Just women:", "Comparing network of men and women:") and those labels are
  kept as body paragraphs exactly where the print puts them.
  This theme sets ragged-right text with no hyphenation, so no dehyphenation
  was needed; the only line-joining repair was rejoining anchors the print
  split across a line break. Hyperlinks reconstructed from the PDF link
  annotations (anchor colour 0x0000EE / 0x551A8B); none guessed. Page anchors
  <!-- page N --> mark print-page boundaries. Source oddities preserved
  verbatim: "mentioned together a lot of and in many contexts", "Historians
  adding manual ties make up a larger proportion ... explains the high
  proportion", "explanation for low proportion", "Six of Degrees of Francis
  Bacon" in the footer, and the double space in "and so forth.  This is

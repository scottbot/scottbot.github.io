...
post_date_visible: "September 16, 2019"
...
tags: ["history", "libraries", "media", "scholarly communication"]
...
archive_timestamp: "20191209075208"

language: "en"
comments_preserved: false
comment_count: 0

source_pdf: "Weingart - 2019 - Modeling Space Ships from Ocean Liners.pdf"
source_pdf_sha256: "3079bc66361e2d02a7fcaf3136dbb5e995ccca2ce51e9cc928af6de11ded061a"
...
extraction_notes: |
  ...
  4/7/2026, of the Wayback snapshot 20191209075208 of
  scottbot.net/modeling-space-ships-from-ocean-liners/. Print
  header/footer, masthead ("the scottbot irregular" / "data are
  everywhen"), the Jetpack "Related" widget (three related-post cards on
  p10), the byline block and icon, the site footer and the stray
  "css.php" link were stripped as chrome. Post date, category, tags and
  permalink come from the printed p10 byline ("scott b. weingart /
  September 16, 2019 / miscellanea / history, libraries, media,
  scholarly communication" — the last tag wraps over two printed lines
  but is a single tag, scholarly-communication).
  The title printed over two lines ("Modeling Space Ships from Ocean" /
  "Liners") and is rejoined in the H1 and YAML title.
  Ligature control characters restored per font (Merriweather-Regular
  \x01=fi \x03=ff \x04=ffi \x05=fl; Merriweather-Italic \x01=ff
  \x02=fl).
  All five section headings ("Caveats", "Context", "Crisis", "Choice",
  "Caveats #2") print at 17.25pt Merriweather-Black, the theme's h3 size
  — the post uses no h2 — so they are rendered as "###" to preserve the
  source's heading level rather than promoted.
  Superscript footnote references were separate text objects in the
  print and were re-inserted inline at their printed x-position as [^N];
  the five "Notes:" entries became Markdown footnote definitions and
  their ↩ back-arrows were stripped.
  Hyperlinks reconstructed from the PDF link annotations with the
  /web/20191209075208/ Wayback prefix stripped.
  Images: 4 content rasters, each followed by its italic caption
  (reproduced below the image): the Bodleian plan (p3), the Hunt Library
  commons (p5), a reading room via flickr (p8), and a screenshot of the
  critical tweet (p9, caption on p10). The tweet is an image in the
  source (its words are not in the print's text layer); its visible text
  is transcribed in that image's alt text, and the poster's name and
  handle are cropped out of the screenshot itself, as the following
  paragraph explains.
...
![Plan of the Bodleian Library, Oxford.](images/img-001.png)
...
![The commons area of the James B. Hunt Jr. Library.](images/img-002.png)
...
![A library reading room lined with books.](images/img-003.png)
...
![Screenshot of a tweet, the poster's name and handle cropped away: “if your blog post begins this way, maybe don't write the blog post but use that time to read the literature instead??” — quoting a highlighted screenshot of the post's own Caveats section. Timestamped 10:03 AM · Sep 16, 2019 · TweetDeck.](images/img-004.png)

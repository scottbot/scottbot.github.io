...
authors:
  ...
  - family: "Eichmann-Kalwara"
    given: "Nickoal"
    affiliation: "University of Colorado, Boulder, US"
    email: "nickoal.eichmann@colorado.edu"
    corresponding: true

publication_type: "article"
container_title: "Digital Studies/Le champ numérique"
...
url: "https://doi.org/10.16995/dscn.284"
language: "en"
license: "CC-BY 4.0 (Creative Commons Attribution 4.0 International License) - © 2017 The Author(s), open access"

abstract: |
  This study identifies how the flagship Digital Humanities conference has
...
abstract_fr: |
  Cette étude a pour but de cerner comment la conférence phare sur les
  humanités numériques a évolué depuis 2004 et continue à évoluer, en analysant
  les tendances thématiques, régionales et d’auteur dans ses présentations. De
  plus, nous explorons dans quelle mesure les humanistes numériques sont à la
  hauteur de la caractérisation en matière de diversité, de collaboration et de
  mondialisation, en utilisant la conférence comme intermédiaire. Étant donné
  la vulgarisation croissante des « humanités numériques » au cours de la
  dernière décennie, et en particulier les récents succès dans la presse
  populaire et les initiatives de subvention, cette étude modère la rhétorique
  parfois utopique qui apparaît aux côtés des mentions du terme.
keywords: ["ADHO", "authorship", "disciplinarity"]
keywords_fr: ["ADHO", "authorship", "disciplinarité"]

source_pdf: "Weingart and Eichmann-Kalwara - 2017 - What’s Under the Big Tent A Study of ADHO Conference Abstracts.pdf"
source_pdf_sha256: "08e94b74b4179005253d1191e74e326671296989a179c59c50ce471d5b1ea56b"
page_count: 18
extraction_date: "2026-08-02"
extraction_tool: "claude scholarly-pdf-to-markdown skill"
extraction_notes: |
  Digital-native typeset journal article (Adobe InDesign CC 2014), single
  column, no OCR. Peer-reviewed research article in Digital Studies/Le champ
  numérique, published by the Open Library of Humanities. All metadata above is
  transcribed from the file itself - the publisher cover sheet (PDF p. 1), the
  title page (PDF p. 2) and the end-of-article colophon (PDF p. 18) - because
  the network/Crossref is not reachable from this environment; the printed DOI
  is 10.16995/dscn.284 and matches the supplied CV entry.

  Publisher chrome removed and recorded here rather than in the body (the exact
  removed strings are deliberately NOT reproduced in these notes, so that leak
  detection stays meaningful): (a) the whole of PDF page 1, the Open Library of
  Humanities cover sheet - article-type label, a how-to-cite block, the
  publication date, and the peer-review, copyright, open-access and digital-
  preservation statements - whose substance is captured in the frontmatter
  fields above; (b) the repeated how-to-cite header at the top of PDF page 2;
  (c) the running head (author names plus short title) and the printed folio on
  every page from PDF p. 3 on; (d) the end colophon on PDF p. 18 - a how-to-cite
  block, the submitted/accepted/published dates, the copyright and CC-BY
  paragraph, the open-access badge image and the journal statement - whose dates
  and licence are in the fields above. Nothing else was dropped.

  Page anchors are the PRINTED folios (1-17), which run one behind the PDF page
  index (printed p. 1 = PDF p. 2); the cover sheet is unnumbered.

  Headings: the journal sets two distinct styles - 12pt upright bold
  (Introduction, Findings, Conclusions and Future Analysis, Author Typology,
  Competing Interests, References) and 11pt bold-italic (Methods and Data,
  Topics, Authorship, Regional Diversity, Gender Distribution). The two levels
  are preserved as "##" and "###" respectively. Note that this typographic
  hierarchy is the publisher's and is not perfectly logical - it makes "Methods
  and Data" a second-level head following "Introduction" - but the printed
  styling is reproduced rather than silently regularised.

  Front matter: the article prints no "Abstract" heading, so the single
  "## Abstract" heading is editorial (playbook convention) and covers both the
  English abstract and the French résumé, which follow each other on the title
  page with their respective Keywords/Mots-clés lines, verbatim and in order.
  The superscript affiliation markers on the byline and affiliation lines are
  preserved as &lt;sup&gt;1&lt;/sup&gt;, &lt;sup&gt;2&lt;/sup&gt;.

  Figures: eight figures, each an embedded 200-DPI raster (the resolution at
  which the publisher placed them), extracted at native resolution rather than
  re-cropped, and named images/fig-001.png … images/fig-008.png in printed
  order (Figure 1 … Figure 8). Captions ("**Figure N:** …", bold label as
  printed) are reproduced verbatim below each image. Chart-internal text (axis
  labels, legends, data labels) rides inside the images and is not transcribed
  as prose. Figures printed at the top of a page that interrupt a paragraph
  running from the previous page are placed after that paragraph ends, so no
  sentence is split: this moves Figures 3 and 4 (printed at the top of p. 9),
  Figure 5 (top of p. 10) and Figures 6 and 7 (top of p. 11) a short distance
  down relative to the printed page.

  Typography: in-text figure cross-references are bold in the source
  ("**Figure 1**") and stay bold; reference-list entries print the author
  element in bold and this is preserved; italics preserved throughout. The
  superscript footnote markers (5.8pt) are placed at their exact source
  positions as [^N]; the ten footnotes are printed at the foot of the page on
  which they are cited and are collected under "## Notes" with their original
  continuous numbering 1-10. The article's own typo "identifed" (Author
  Typology section) is left as printed. The Author Typology contribution list
  is printed as one unbulleted line per role and is rendered one line per role.

  Joins: a single true line-break hyphenation ("Archa-|eological", p. 15 of the
  print) was rejoined and the hyphen dropped; all other line-final hyphens are
  real compound hyphens (well-defined, self-identified, multi-authored,
  self-definitions) or occur inside URLs and were rejoined with the hyphen
  kept. URLs broken across lines by the typesetter were rejoined without an
  inserted space. Soft hyphens and Unicode spaces normalised; the source's
  curly quotes, guillemets, en/em dashes, ≈, ± and accented characters are
  preserved as printed. MANIFEST.txt included.
